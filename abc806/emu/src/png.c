// abc806/emu/src/png.c - a minimal PNG writer, so a rendered ABC806 screen
...
bool abc806_write_png(const char *path, const uint8_t *pixels, int width, int height,
    ...
    if (!path || !pixels || !palette || palette_size <= 0) return false;
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            int idx = pixels[(size_t)y * (size_t)width + (size_t)x];
            uint32_t c = palette[idx < palette_size ? idx : 0];
            row[1 + (size_t)x * 3 + 0] = (uint8_t)(c >> 16);
            row[1 + (size_t)x * 3 + 1] = (uint8_t)(c >> 8);
            row[1 + (size_t)x * 3 + 2] = (uint8_t)c;
        }
    }
