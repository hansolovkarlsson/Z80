...
static gboolean on_key_pressed(GtkEventControllerKey *controller, guint keyval,
    ...
    // Ctrl-] or F12 stops in the debugger, as in bin/abc802 --interactive
    // (Z80DBG_BREAK_CHAR, Z80DBG_BREAK_CSI_PARAMS). Without a debugger
    // neither is a key the ROM uses.
...
// started from. The machine keeps its window live meanwhile, because the
// debugger runs in async mode and only this callback feeds it.
...
int main(int argc, char *argv[]) {
    ...
    if (app.dbg) {
        ...
        z80dbg_add_space(app.dbg, &(Z80DbgSpace){"chr", "character RAM, 7800-7FFF to data reads",
                                                 ABC802_CHAR_RAM_SIZE, abc802_char_ram_peek, abc802_char_ram_poke});
        z80dbg_add_space(app.dbg, &(Z80DbgSpace){"lowram", "RAM at 0000-7FFF, wherever LRS has put it",
                                                 0x8000, abc802_low_ram_peek, abc802_low_ram_poke});
