...
printf 'r\nd all\nc\n' > "$DBG_SCRIPT"
out=$("$ABC802" --screen --cycles "$CAP" --type $'PRINT 6*7\r' \
      --break 3A76 --debug-script "$DBG_SCRIPT" 2>&1)
tl_begin "debugger-break"
tl_want "$out" "[breakpoint] 3A76" "the stop at 3A76"
tl_want "$out" "PC=3A76" "the registers shown at the stop"
