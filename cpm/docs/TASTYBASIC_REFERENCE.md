...
Basic](https://github.com/dimitrit/tastybasic) (`cpm/resources/tastybasic/`), the
real CP/M program this emulator has been validated against — see
`cpm/docs/CPM_REFERENCE.md`'s Implementation status section and
`cpm/docs/ROADMAP.md` for how that testing surfaced and fixed real bugs in
...
`cpm/resources/tastybasic/upstream/README.md`.

Gathered from Tasty Basic's own command tables (`tab1`/`tab2`/`tab4`/
`tab5`/`tab6`/`tab8` in `cpm/resources/tastybasic/upstream/tastybasic.asm`) and its
...
```
bin/z80asm cpm/resources/tastybasic/tastybasic_cpm.asm -o tastybasic.com
...
`cpm/docs/ROADMAP.md`/git history for the fixes these prompted on the
...
- **Filenames are capped at 7 characters, not 8.** `fname:`'s length
  check (`cpm/resources/tastybasic/upstream/cpmio.asm`) decrements its counter
...
- **`LOAD` silently truncates a program at any line number whose *low*
  ...
  bit the CP/M example programs bundled in `cpm/resources/tastybasic-main/`
