...
emulation (`cpm/emu/src/cpm.c`) targets, gathered from the CP/M 2.2 Interface
...
document is the Phase 3 groundwork item from `cpm/docs/ROADMAP.md`: pin down
...
bug (BDS C's CDB debugger; see `cpm/docs/ROADMAP.md`) surfaced that only four
...
`cpm/docs/ROADMAP.md`'s Phase 3 list).
...
similarly gets a real `JP <BDOS_ENTRY>` (`BDOS_ENTRY` in `cpm/emu/src/cpm.h`,
...
(`cpm/resources/ccp/upstream/ccp.asm`'s `bmove0`/`bmove1`/`bmove2`), not
...
`cpm/emu/src/cpm.c`'s `check_cpm_bdos()` currently implements **0**
...
redirected stdin — e.g. `cpm/tests/run_tests.sh`'s `console_test.asm` check —
...
- Sequential I/O (`F_READ`/`F_WRITE`) tracks position via the FCB's real
  ...
  own data-file paging is what surfaced this — see `cpm/docs/ROADMAP.md`).
...
- **The fake DPH/DPB** (`DPH_BASE`/`DPB_BASE`/`DIRBUF_BASE`/`ALV_BASE` in
  ...
  (see `cpm/docs/TURBOPASCAL_REFERENCE.md`) — **confirmed fixed**, now
...
running a single program — see `cpm/resources/ccp/` for the genuine,
...
`cpm/docs/ROADMAP.md`'s "Get a real CP/M 2.2 CCP booting" entry for the full
...
`cpm/docs/CCP_REFERENCE.md` for the built-in commands (`DIR`/`ERA`/`TYPE`/
