...
A reference for Borland's Turbo Pascal 3.01A — `cpm/resources/turbopascal/`,
...
validated against (see `cpm/docs/ROADMAP.md`'s "Real-world validation: Turbo
...
`cpm/docs/ROADMAP.md`): `ClrScr`, `ClrEol`, `GotoXY(X,Y)`, `InsLine`,
...
- **`Ctrl-H`/Backspace/Delete used to move the cursor left without
  ...
  **Fixed** by using Turbo Pascal's own reconfiguration tool the way real
  users would: `cpm/resources/turbopascal/derive.sh` now also runs
...
- **Ships configured for a "Microbee VDU" terminal**, not ANSI — this
  project's `cpm/resources/turbopascal/derive.sh` reconfigures it via
  `TINST.COM`'s real "ANSI" profile before use. See
  `cpm/resources/turbopascal/upstream/README.md`.
- **A real emulator bug, found compiling a genuinely large source file
  (`cpm/resources/hanoi/`) — now fixed.** Loading or compiling any work/main
  ...
  shadowing it with a second one. Verified with `cpm/resources/hanoi/` (see
...
- **A genuine, real Turbo Pascal 3.01A compiler limit, hit by a second
  third-party program (`cpm/resources/queens/`) — not an emulator bug, and
  ...
  `cpm/resources/queens/` therefore uses the upstream project's own prebuilt
  `queens.com`/`queens.dat` rather than compiling from source (unlike
  `cpm/resources/hanoi/`) — see `cpm/resources/queens/upstream/README.md`. The
...
program from scratch: `cpm/resources/hanoi/upstream/hanoi-p.pas` (Francesco
...
real banner, menu, and board — see `cpm/resources/hanoi/derive.sh`. And a
third: `cpm/resources/queens/upstream/queens.com` (Francesco Sblendorio's
