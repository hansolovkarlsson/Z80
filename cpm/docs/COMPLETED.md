# CP/M target: completed phases

Finished work on `bin/z80`, `bin/z80asm` and `bin/z80dasm`,
moved here out of `ROADMAP.md` so that file stays a short answer to "what
works, what doesn't, and what's next" rather than a growing archive.

Nothing here has been rewritten — these are the phase write-ups as they
were recorded when the work was done. They are worth keeping in full
because most of this project's hardest-won knowledge is in them: the real
CP/M behaviors that only surfaced when a real piece of 1980s software
refused to work, and the investigations that traced each one back to its
actual cause.

One correction made in the move: **Phase 2's heading still said "in
progress" although every item under it is checked off** and the assembler
has since gone well past its stated goal (it assembles the real,
unmodified ZSM4 `.mac` sources, not just the generated `.z80` ones). The
heading was stale, not the status.

For the CP/M 2.2 spec itself see `CPM_REFERENCE.md`; for current status
and open gaps see `ROADMAP.md`; for the cross-cutting lessons see
`../../docs/JOURNAL.md` and `../../docs/postmortems/`.
...
## Core gaps closed after Phase 1 (moved out of the roadmap)

These five sat in `ROADMAP.md`'s "Known gaps" section as checked-off
boxes long after they were finished — the same drift the machine targets'
roadmaps had. Moved here verbatim; nothing has been rewritten.

They share a theme worth stating once: **ZEXALL and ZEXDOC exercise none
of them.** The exercisers test instruction results and flags, and have
nothing to say about I/O ports, interrupt modes, interrupt delivery, or
`RETI`/`RETN`. `asm/examples/gaps_test.asm` and
`cpm/tests/test_interrupts.c` exist precisely to cover what the oracle
cannot, which is also how the shared core's missing block-I/O opcodes
survived until an ABC802 ROM drove them — see
[`../../docs/postmortems/2026-08-28-block-io-opcodes-missing.md`](../../docs/postmortems/2026-08-28-block-io-opcodes-missing.md).
...
step: a TextMate grammar for colouring, and completion of mnemonics,
directives, registers, conditions and the file's own labels, constants and
macros, including those from `INCLUDE`d files. `asm/vscode/install.sh`
packages it as a `.vsix` with Python's zipfile and installs it; linking the
folder into `~/.vscode/extensions` does nothing, since VS Code loads only
what its registry lists.

**It follows the assembler, not Z80 syntax in general.** The keyword lists
are generated by `build_grammar.py` and are a copy of `asm/src`'s, so the
tests compare them with the source both ways; the first run found `SL1`,
an alias of `SLL` the lists had missed. The symbol scanner is compared with
`z80asm -s` on every example and on `zexall.mac`, and its first run found
that a macro defined in an `INCLUDE`d file was being read as a label where
it was called: the assembler expands macros across the whole include graph
before it reads a label, so the scanner now gathers every file first. A
`'` after a letter, digit or `_` does not open a string, as in the
assembler, which with the `AF'` register rule makes two independent
defences for `EX AF,AF'`; the test fails only when both are removed.

**Tested on this machine with no Node installed**: VS Code's own Electron
runs as Node, and the grammar is tokenised by VS Code's own bundled
`vscode-textmate` and Oniguruma, so the engine under test is the one that
colours the editor. `extension.js` is exercised through a stand-in for the
`vscode` module rather than a live window. Five checks, each seen to
fail: the mnemonic list (on `SL1`), the directive list (with `REPT`
removed), the scanner (on the include-graph macros), the grammar (with its
