...
// 6, 10, 11 - see cpm/docs/CPM_REFERENCE.md) see input the same way real CP/M
...
// ANSI-enhanced SARGON port in cpm/resources/sargon/sargon78.com) commonly
...
// screen:
//   - dBASE II's own APPEND/EDIT/BROWSE (`cpm/cpm_disk/DBASE.COM`) targets a
...
//     (`cpm/resources/bdsc/upstream/README.md`) and built from its real
 ...
 * Disk Parameter Block per drive (see cpm/docs/CPM_REFERENCE.md) - modeling
 ...
 * naming "FOO.TXT" maps straight to "cpm_disk/FOO.TXT" on the host,
 * relative to wherever bin/z80 is invoked from - not necessarily the
 * repo root. The checked-in sample disk directory this project ships is
 * at cpm/cpm_disk/, which is why the documented convention (CLAUDE.md,
 * README.md) is to invoke bin/z80 from inside cpm/, not the repo root -
 * see CLAUDE.md's top-level layout. This can't express CP/M's
 * drive-switching or per-user file areas, but covers what the vast
 * majority of CP/M-80 transient programs actually need from BDOS file
 * calls, without an on-disk format to get bit-exact.
 */
 ...
 * `JP <wboot>` instruction (see cpm/docs/CPM_REFERENCE.md's zero-page map),
...
// cpm/docs/CPM_REFERENCE.md's DPB section), so the actual number just needs
