...
`cpm/gtk/src/main.c` is a thin GTK4 launcher for the real `bin/z80` CLI
...
VT100/ANSI interpretation. `z80.c`/`cpm.c`/`cpm/emu/src/main.c` are untouched
...
(`make gtk` runs from the repo root, same as any other Makefile target;
`bin/z80-gtk` itself needs to run from inside `cpm/` for its `cpm_disk/`
argument to resolve, same convention as `bin/z80` - see the top-level
`README.md`.)
...
running `../bin/z80-gtk emu/zexall/ZEXALL-main/zexall.com` (from inside
`cpm/`) shows the real `VteTerminal` widget with `bin/z80`'s own output
rendering correctly. Two
