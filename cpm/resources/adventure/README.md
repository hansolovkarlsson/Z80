...
(unlike SARGON in `cpm/resources/sargon/`) - only the compiled binary and its
...
**License note**: like `cpm/resources/sargon/`, this is old commercial/
...
Both files are already in `cpm/cpm_disk/` (see that directory's own
README) - simplest is just:
...
Or run from any other directory whose own `cpm_disk/` contains both
files (see `CLAUDE.md`'s File I/O section for how drive/user numbers map
onto that one host directory):
