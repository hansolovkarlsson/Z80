...
`docs/JOURNAL.md` holds *why*, in the order it happened, and
`docs/postmortems/` (indexed by its own `README.md`) holds the failures
whose lesson outlived the fix. The "what is left" / "what was built"
ledger is kept per machine target rather than once for the repo:
`cpm/docs/ROADMAP.md` with `cpm/docs/COMPLETED.md`, and the same pair
under `abc80/docs/`, `abc802/docs/` and `abc806/docs/` under prefixed
names. There is deliberately no changelog; `docs/JOURNAL.md`'s own
opening note says why. The reference documents listed in the Project
section below (`docs/Z80_REFERENCE.md`, `docs/ASSEMBLER.md`, each
target's `*_REFERENCE.md`) describe the machines and the tools, not the
work, and are a separate set from these.

Each of those documents opens with a note stating its own job, and that
note is the specification for what belongs in it: follow it over any
general instruction, this paragraph included. Everything written here is
a copy of something written there, so where the two disagree the document
itself is right.
...
disassembler in `disasm/src/`, and the ABC floppy-image tool in
...
make test          # build and run all four suites (see below)
...
make test-abc806   # just abc806/tests/run_tests.sh
make clean         # remove object files and every binary above
```
...
primitives in `scripts/testlib.sh`. Where a check can be written as BASIC it
is, since `OUT`/`INP`/`PEEK` make the emulated machine its own test harness
and so exercise the real port decode and the ROM's own implementation rather
than a C-level shortcut around both. The floppy checks need real disk images
this repo deliberately does not commit; set
`ABC80_TEST_DISKS`/`ABC802_TEST_DISKS`/`ABC806_TEST_DISKS` to a directory
holding them and they run, and otherwise they **skip loudly** and are
counted separately from passes. Three ASCII-art fixtures
(`*/tests/fixtures/chargen.txt`, regenerated by each target's own
`regen-fixtures.sh`) cover the character generator decodes, which no boot
screen exercises. The suites were validated by deliberately injecting six
real regressions and confirming each was caught — that sweep immediately
found one check asserting on the *echoed command line* rather than on
BASIC's answer, which passed with its subject entirely broken.
...
decoder recognises it; on a Mac whose top row sends media keys it needs
Fn). On a Swedish layout a terminal delivers Ctrl on the key right of Å
as Ctrl-], so the terminal needs nothing more. The GTK windows do: they
get no function keys from GTK's macOS backend and see that key by its
code, so they also take Cmd-. and Ctrl on the key where a US layout has
`]` (matched by key code, 30 on macOS),
