...
if tl_gate "$gtk_skip_reason" gtk-headless-boot gtk-headless-type gtk-debugger; then
    ...
    out=$(cd "$ROOT/abc80" && python3 "$ROOT/scripts/ptysession.py" \
          'wait:1,text:b 031E,wait:0.5,text:c,wait:0.5,text:r,wait:0.3,text:c,wait:0.5,text:r,wait:0.3,text:d all,wait:0.3,text:c,wait:3' \
          -- "$GTK_BIN" resources/rom --steps 400000 --screenshot "$GTK_TMP/debug.png" --debug 2>&1)
    tl_begin "gtk-debugger"
    tl_want_eq "$(printf '%s\n' "$out" | grep -c '^\[breakpoint\] 031E')" "2" "two stops at 031E"
...
out=$(cd "$ROOT/abc80" && python3 "$ROOT/scripts/ptysession.py" \
      ...
      -- "$ABC80" --interactive --symbols "resources/rom/abc80.sym" 2>&1)
tstates=$(printf '%s\n' "$out" | sed -n 's/^Total T-states: *\([0-9]*\).*/\1/p' | tail -1)
report=$(printf '%s\n' "$out" | grep '^ptysession:')
verdict=$(printf '%s\n' "$report" | tr ' =' '\n\n' | awk -v t="${tstates:-0}" -v hz=2995200 '
