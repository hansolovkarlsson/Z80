# ABC80: completed milestones

Finished work on `bin/abc80` and `bin/abc80-gtk`, moved here out
of `ABC80_ROADMAP.md` so that file stays a short answer to "what works,
what doesn't, and what's next" rather than a growing archive — it had
reached roughly 2,900 lines, of which nearly all was completed-work
narrative.

Nothing here has been rewritten. These are the milestone write-ups as they
were recorded when the work was done, and they are worth keeping in full:
much of the value is in the dead ends, the wrong first guesses, and the
"found the hard way" notes, which is exactly the material a summary would
throw away.

One correction made in the move: **Milestone 6's heading did not say
"done" although every item under it is checked off** and its remaining
loose ends were explicitly closed out. The heading was stale, not the
status.

For the hardware being modeled see `ABC80_REFERENCE.md`, for the BASIC
dialect `ABC80_BASIC_REFERENCE.md`; for current status and open gaps see
`ABC80_ROADMAP.md`; for the cross-cutting lessons see
...
## Milestone 6: ABCbus expansion — done
