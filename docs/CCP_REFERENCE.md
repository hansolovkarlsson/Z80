...
`resources/ccp/`, genuine, unmodified Digital Research source (see
`resources/ccp/upstream/README.md` for provenance and
`docs/ROADMAP.md`'s "Get a real CP/M 2.2 CCP booting" entry for how it
got running here, including two real bugs that surfaced along the way).
Unlike `docs/TASTYBASIC_REFERENCE.md`/`docs/MBASIC_REFERENCE.md`, this
document describes the actual DRI CCP source directly (read from
`resources/ccp/ccp_cpm.asm`), not a language built on top of it — the
...
(`P_TERMCPM`) all correctly land back at `A>` (see `docs/ROADMAP.md` for
...
- **Serialization check disabled.** The real DRI source includes an
  ...
  `resources/ccp/preprocess.py`'s own comment and
  `docs/ROADMAP.md`.
...
`docs/ROADMAP.md`, correct 4-per-line wrapping), `TYPE`, and running
