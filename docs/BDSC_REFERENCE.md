...
A reference for [BDS C](https://www.bdsoft.com/resources/bdsc.html) v1.60,
Leor Zolman's real 8080/Z80 C compiler for CP/M-80 (`resources/bdsc/`) —
**public domain** since September 20, 2002 (the author released all
rights explicitly, source included; see
`resources/bdsc/upstream/README.md`). Unlike every other third-party
...
them with `resources/bdsc/derive.sh`, which compiles and links
`resources/bdsc/examples/hello.c` and `fib.c` through the real
...
see `docs/ROADMAP.md`'s Phase 3 entry and `CLAUDE.md`'s File I/O
...
`resources/bdsc/derive.sh` greps for `to spare` and `link space
...
`CLAUDE.md`'s File I/O section and `docs/CPM_REFERENCE.md`'s
...
The full distribution (see `resources/bdsc/upstream/README.md` for
