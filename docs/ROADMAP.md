...
- [ ] **Real ZSM4 compatibility target: assemble `zexall.mac`/`zexdoc.mac`,
  not just `zexall.z80`/`zexdoc.z80`.** `zexall.z80` is *not* a
  human-maintained ZSM4 source — its own header says `; zexlax.z80`, and
  it's generated by `emu/zexall/ZEXALL-main/zexlax.pl` (a Perl script that
  fills `@c`/`@d`/`@f`/`@m`/`@s` placeholders in an embedded template with
  ...
  ZSM4` in the header, meant to be built with actual ZSM4
  (`zsm4 =zexdoc/L`, per `emu/zexall/ZEXALL-main/README.md`). Diffing the
  two (`diff zexall.z80 zexall.mac`) shows they're otherwise nearly
  identical — three real differences, only one of which is a genuine gap:
  1. `.title` / `.z80` (dot-prefixed directives) — currently silently
     no-op (my mnemonic-word extraction doesn't include `.` as a leading
     character, so these lines fall through the "not a label, not a
     mnemonic" path and get skipped harmlessly). Works today more by
     accident than design; worth making explicit.
  2. A colon-less `EQU` label (`flgsat equ spat-2`, no `:`) — should
     already work via the existing colon-less-label disambiguation
     (`is_known_mnemonic()`), same mechanism that handles `bdos push af`
     already. Worth explicitly verifying against the real file rather than
     assuming.
  3. **The real gap**: a `dss` macro whose body contains a nested `REPT
     bytes ... ENDM` block (`dss` itself is `MACRO ... ENDM`, so `ENDM`
     closes two different things depending on nesting depth). `tstr`/
     `tmsg` call `dss &lab+4-$,0` / `dss &lab+30-$,'.'` in place of the
     plain `DS` directive `zexall.z80` uses instead. This needs two
     things: (a) nesting-aware macro-body capture in `preprocess.c` (right
     now `in_macro_def` is a plain boolean, so the inner `REPT`'s `ENDM`
     would be mistaken for the end of `dss` itself), and (b) `REPT`'s
     repeat count here is `$`-dependent (`bytes` = `&lab+4-$`), which
     means — exactly like `IF`/`ELSE`/`ENDIF` already had to be — `REPT`
     can't be resolved by pure text preprocessing before addresses are
     known; it needs to be integrated into the real two-pass loop in
     `assemble.c`, not `preprocess.c`.
  Reference material for this now lives in `resources/`: `zsm4.pdf` and a
  link to Hector Peraza's ZSM4 project. Once `REPT`/`ENDM` works, attempt
  `zexall.mac`/`zexdoc.mac` directly (same validation approach as the
  `zexall.z80` milestone above: assemble, then run the result through
  `bin/z80` and confirm 67/67 OK) rather than assuming success.
